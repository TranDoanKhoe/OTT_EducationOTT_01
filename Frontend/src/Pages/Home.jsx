...
const Home = () => {
  ...
  const [userId, setUserId] = useState(
    navUserId || localStorage.getItem("userId") || "680e6d95a73e35151128bf65",
  );
  ...
  const filterAiConversationMessages = useCallback(
    (list) => {
      ...
      return arr.filter((msg) => {
        ...
        const isUserToAi =
          senderId === currentUserId && receiverId === AI_ASSISTANT_ID;
        const isAiToUser =
          senderId === AI_ASSISTANT_ID && receiverId === currentUserId;
  ...
  const createNewAiConversation = useCallback(async () => {
    ...
    try {
      const created = await createAiConversation(token, "Cuộc trò chuyện mới");

      setAiConversations((prev) => {
        const safePrev = Array.isArray(prev) ? prev : [];
        return [created, ...safePrev.filter((c) => c.id !== created.id)];
      });
  ...
  }, [selectedContactId, contacts, selectedContact, mergeContactWithSettings]);
      ...
      connectWebSocket(
        ...
        (receivedMessage) => {
          ...
          setMessages((prev) => {
            ...
            let createAt =
              receivedMessage.createdAt || receivedMessage.createAt;
          ...
          if (!isSentByMe && isTypeEnabled) {
            ...
            const senderName =
              contacts.find((c) => c.id === receivedMessage.senderId)?.name ||
              "Người dùng";
            ...
            if (notificationSettings.pushEnabled) {
              showNotificationIfHidden(senderName, {
                body: messagePreview,
                icon: contacts.find((c) => c.id === receivedMessage.senderId)
                  ?.avatar,
                onClick: () => {
                  window.focus();
                },
            ...
            setMessageNotifications((prev) => {
              const notificationId =
                receivedMessage.id || `${contactId}-${Date.now()}`;
              ...
              return [
                {
                  ...
                  preview,
                  createAt:
                    receivedMessage.createAt || new Date().toISOString(),
          ...
          setContacts((prevContacts) => {
            ...
            if (existingContact) {
              ...
              fetchUserById(contactId)
                ...
                .catch((err) => {
                  console.error("Error fetching stranger info:", err);
                });
          ...
          setMessages((prev) =>
            prev.map((msg) =>
              msg.id === deletedMessage.id
                ? {
                    ...msg,
                    deletedByUsers: deletedMessage.deletedByUsers || [],
                  }
                : msg,
            ),
          );
          ...
          setMessages((prev) =>
            prev.map((msg) =>
              msg.id === recalledMessage.id
                ? {
                    ...msg,
                    recalled: recalledMessage.recalled || false,
                  }
                : msg,
            ),
          );
        ...
        (pinnedMessage) => {
          ...
          setMessages((prev) =>
            prev.map((msg) =>
              msg.id === pinnedMessage.id ? { ...msg, isPinned: true } : msg,
            ),
          );
        },
        (unpinnedMessage) => {
          ...
          setMessages((prev) =>
            prev.map((msg) =>
              msg.id === unpinnedMessage.id ? { ...msg, isPinned: false } : msg,
            ),
          );
        },
        groupIds,
        ...
        (callSignal) => {
          ...
          switch (callSignal.type) {
            case "offer":
              ...
              const caller = contacts.find((c) => String(c.id) === senderId);
              const signalName =
                callSignal?.data?.callerName || callSignal?.data?.senderName;
              const signalAvatar =
                ...
                fetchUserById(senderId)
                  .then((user) => {
                    ...
                    setIncomingCall((prev) => {
                      if (!prev || String(prev.senderId) !== senderId) {
                        return prev;
                      }
            ...
            default:
              console.warn("Unknown call signal type:", callSignal.type);
          }
        },
        (readReceipt) => {
          if (!isMounted) return;
          ...
          setMessages((prev) =>
            prev.map((msg) =>
              msg.id === readReceipt.id ? { ...msg, isRead: true } : msg,
            ),
          );
        },
        async () => {
        ...
        async () => {
          if (!isMounted) return;
          await Promise.all([updateFriendsList(), updatePendingRequests()]);
        },
        async () => {
          if (!isMounted) return;
          await updatePendingRequests();
    ...
    try {
      ...
      const groupContacts = await Promise.all(
        groups.map(async (group) => {
          ...
          try {
            const chatHistory = await getGroupChatHistory(group.id, token);
      ...
      setContacts((prev) =>
        ensureAiContact([...prev.filter((c) => !c.isGroup), ...groupContacts]),
      );
      ...
      if (data) {
        const mappedFriends = await Promise.all(
          data.map(async (friend) => {
            ...
            let lastMessage = normalizeLastMessage(friend.lastMessage);
            try {
              const chatHistory = await getChatHistory(friend.id, token);
              if (chatHistory && chatHistory.length > 0) {
                const lastMsg = chatHistory[chatHistory.length - 1];
                if (lastMsg.type === "TEXT") {
                  lastMessage = lastMsg.content;
              ...
              console.error("Error loading last message for", friend.name, err);
            }
      ...
      if (data) {
        ...
        setGroupInvites(
          data.map((invite) => ({
            ...
            groupName: invite.groupName || invite.group?.name || "Nhóm",
            inviterName:
              invite.inviterName || invite.inviter?.name || "Người dùng",
        ...
        if (selectedIsAi) {
          ...
          if (!targetConversationId) {
            if (list.length > 0) {
              ...
              setMessages(filterAiConversationMessages(loadAiHistory()));
              return;
            }
          }
          ...
          const serverMessages = Array.isArray(conversation?.messages)
            ? conversation.messages
            : [];
          const normalizedServer = filterAiConversationMessages(serverMessages);
  ...
  useEffect(() => {
    if (selectedContact?.isAI) {
      ...
      localStorage.setItem(aiHistoryStorageKey, JSON.stringify(aiOnlyMessages));
      ...
      if (token && currentAiConversationId) {
        saveAiConversationMessages(
          ...
          .catch((error) => {
            console.error("Error syncing AI history to server:", error);
          });
      }
    }
  }, [
    ...
    (message) => {
      ...
      setMessages((prev) => {
        ...
        const persistAiHistoryIfNeeded = (nextMessages) => {
          ...
          try {
            const aiOnlyMessages = filterAiConversationMessages(nextMessages);
            localStorage.setItem(
              aiHistoryStorageKey,
  ...
  useEffect(() => {
    const searchUsers = async () => {
      ...
      try {
        ...
        const userData = await fetchUserByPhone(headerSearchQuery.trim());
  ...
  const handleAcceptCall = async () => {
    ...
    try {
      ...
      // Get local media
      const stream = await startCall(incomingCall.data?.isVideoCall || false);
      ...
      if (error.message.includes("quyền truy cập")) {
        ...
        setSnackbarMessage("Không thể chấp nhận cuộc gọi: " + error.message);
        setSnackbarSeverity("error");
        setOpenSnackbar(true);
      }
      handleEndCall();
  ...
  const handleStartCallFromFriendsList = async (friend, isVideo) => {
    ...
    setTimeout(async () => {
      try {
        ...
        initializePeerConnection(
          (candidate) => {
            sendCallSignal("ice-candidate", candidate, friend.id, token);
          },
          (stream) => {
            setRemoteStream(stream);
        ...
        if (error.message.includes("quyền truy cập")) {
          ...
          setSnackbarMessage("Không thể bắt đầu cuộc gọi: " + error.message);
          setSnackbarSeverity("error");
          setOpenSnackbar(true);
        }
        handleEndCall();
  ...
  const handleSendFriendRequest = useCallback(async () => {
    ...
    try {
      ...
      setSnackbarMessage(
        "Lỗi gửi lời mời kết bạn: " + (error.message || "Không xác định"),
      );
      setSnackbarSeverity("error");
      setOpenSnackbar(true);
    } finally {
  ...
  const handleAcceptFriendRequest = useCallback(
    async (requestId) => {
      ...
      try {
        ...
        setSnackbarMessage(
          "Lỗi chấp nhận lời mời: " + (error.message || "Không xác định"),
        );
        setSnackbarSeverity("error");
        setOpenSnackbar(true);
      } finally {
  ...
  const handleRejectGroupInvite = useCallback(
    async (inviteId) => {
      ...
      try {
        ...
        setSnackbarMessage(
          "Lỗi từ chối lời mời: " + (error.message || "Không xác định"),
        );
        setSnackbarSeverity("error");
        setOpenSnackbar(true);
      } finally {
  ...
  const handleCreateGroup = async () => {
    ...
    try {
      ...
      setSnackbarMessage(
        "Lỗi tạo nhóm: " + (error.response?.data?.message || error.message),
      );
      setSnackbarSeverity("error");
      setOpenSnackbar(true);
    } finally {
  ...
  const handleDeleteConversation = useCallback(
    async (contactId, options = {}) => {
      const isAiConversation = options?.isAI || contactId === AI_ASSISTANT_ID;
      ...
      <div className="flex h-[100dvh] overflow-hidden pb-[calc(68px+env(safe-area-inset-bottom))] lg:pb-0">
        {showSidebarNav && (
          <NavSidebar
            ...
            notificationCount={pendingRequests.length + groupInvites.length}
          />
        )}
        {currentView === "notifications" ? (
          <NotificationPanel
            ...
            onMarkMessageNotificationRead={handleMarkMessageNotificationRead}
            onDeleteMessageNotification={handleDeleteMessageNotification}
          ...
          <ResourcesPanel onClose={() => setCurrentView("messages")} />
        ) : currentView === "classes" ? (
          <ClassPanel
            token={token}
            userId={userId}
          ...
          >
              ...
              {contactView !== "all" ? (
                <div className="flex items-center">
                  ...
                  <h6 className="font-bold text-lg text-gray-800">
                    {contactView === "friends" && "👥 Danh sách bạn bè"}
                    {contactView === "groups" && "📚 Lớp học & Nhóm"}
                    {contactView === "friend-requests" && "✉️ Lời mời kết bạn"}
                    {contactView === "group-invites" && "📨 Lời mời vào nhóm"}
                  </h6>
                </div>
              ) : (
              ...
              <div className="bg-white px-4 py-3 border-b border-gray-100">
                <div className="flex items-center justify-between">
                  ...
                  <div className="relative">
                    <select
                      value={messageTab}
                      onChange={(e) => setMessageTab(e.target.value)}
                  ...
                  >
                    ...
                    >
                      <div className="w-9 h-9 rounded-xl bg-emerald-100 flex items-center justify-center mr-3 group-hover:bg-emerald-200 transition-colors">
                        <BiUserPlus size={18} className="text-emerald-600" />
                      </div>
                      <div>
                    ...
                    >
                      <div className="w-9 h-9 rounded-xl bg-violet-100 flex items-center justify-center mr-3 group-hover:bg-violet-200 transition-colors">
                        <BiGroup size={18} className="text-violet-600" />
                      </div>
                      <div>
                        <p className="font-medium text-gray-800">
                          Lớp học & Nhóm
            ...
            {currentView === "messages" && contactView === "all" && (
              <>
                {/* Hiển thị kết quả tìm kiếm người dùng mới */}
                {headerSearchQuery && searchResults.length > 0 && (
                  <div>
                        ...
                        >
                          ...
                          {(user.friendStatus === "NONE" ||
                            ...
                            <button
                              onClick={() => {
                                ...
                                setSelectedContact(strangerContact);
                                setHeaderSearchQuery("");
                              }}
                          ...
                          {user.friendStatus === "NONE" && (
                            <button
                              onClick={async () => {
                                try {
                                  await sendFriendRequest(user.phone, token);
                                  setSnackbarMessage("Đã gửi lời mời kết bạn!");
                                  setSnackbarSeverity("success");
                                  setOpenSnackbar(true);
                                  setSentFriendRequests(
                ...
                {headerSearchQuery &&
                  ...
                  filteredContacts.length === 0 &&
                  /^\d+$/.test(headerSearchQuery.trim()) && (
                    <div className="p-8 text-center">
                      <p className="text-sm text-gray-600">
                        Số điện thoại chưa đăng ký hoặc không cho phép tìm kiếm
                      </p>
                    </div>
                  )}
            ...
            {currentView === "messages" && contactView === "friends" && (
              <div className="h-full overflow-auto">
                <p className="text-sm text-gray-600 p-4 pb-2">
                  {filteredContacts.filter((c) => !c.isGroup).length} bạn bè
                </p>
                <FriendsList
                  contacts={filteredContacts}
            ...
            {currentView === "messages" && contactView === "groups" && (
              <div className="h-full overflow-auto">
                <p className="text-sm text-gray-600 p-4 pb-2">
                  {filteredContacts.filter((c) => c.isGroup).length} nhóm
                </p>
                <ContactList
                  contacts={filteredContacts.filter((c) => c.isGroup)}
                  selectedContact={selectedContact}
                  onContactSelect={setSelectedContact}
                  pendingRequests={[]}
                  onAcceptFriendRequest={handleAcceptFriendRequest}
            ...
            {currentView === "messages" && contactView === "group-invites" && (
              <div className="p-8 text-center mt-8">
                <p className="text-sm text-gray-600">Chưa có lời mời nào</p>
              </div>
            )}
            <SettingsPanel
