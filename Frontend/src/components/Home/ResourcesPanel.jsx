...
const ResourcesPanel = () => {
    ...
    const handleShare = async (resource) => {
        ...
        if (
            shareCandidates.user.length > 0 ||
            shareCandidates.group.length > 0
        ) {
            return;
        }
    ...
    const handleConfirmShare = async () => {
        ...
        try {
            ...
            const isFolder =
                sharingResource.isFolder === true ||
                sharingResource.folder === true;
            const sharedName =
                sharingResource.name || sharingResource.fileName || 'Tài liệu';

            const messagePayload = {
                senderId: currentUserId,
                [shareTargetType === 'group' ? 'groupId' : 'receiverId']:
                    shareTargetId,
    ...
    const getItemCount = (folderId) => {
        return resources.filter((r) => (r.parentId || null) === folderId)
            .length;
    };
                    ...
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center h-full">
                            ...
                            <p className="text-gray-500 text-center max-w-md mb-6">
                                Lưu trữ, quản lý và chia sẻ tài liệu học tập với
                                bạn bè và nhóm học của bạn.
                            </p>
